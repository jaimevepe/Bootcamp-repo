 With the same setup as the previous SQL question (which has Aaron Brooks), write a single SQL Query that returns data from the `TEAMS` and `PLAYERS` tables such that
